...
export async function importFileEnhanced(
  ...
  try {
    const response = await http.post<{ success: boolean; message: any }>('/api/data_importer/_import_file_enhanced', {
      body: formData,
      headers: {
...
export async function previewFileEnhanced(
  ...
  const response = await http.post<EnhancedPreviewResponse>('/api/data_importer/_preview_enhanced', {
    body: formData,
  });
...
export class ImportProgressTracker {
  private onProgress: (progress: { processed: number; total: number; percentage: number; status: string }) => void;
...
export class ImportPerformanceMonitor {
  ...
  getPerformanceReport(): {
    ...
    if (this.metrics.fileSize > 50 * 1024 * 1024) { // 50MB
